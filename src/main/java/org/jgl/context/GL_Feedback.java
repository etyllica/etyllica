...
public class GL_Feedback {
    ...
    public void write_feedback_token (float token) {
    	if (BufferCount < BufferSize) { Buffer [BufferCount] = token; }
	BufferCount++;
    }
...
//				       float texcoord []) {
	...
	if ((Mask & FB_3D) != 0 ) { write_feedback_token (z); }
	if ((Mask & FB_4D) != 0 ) { write_feedback_token (w); }
...
/*	if ((Mask & FB_TEXTURE) != 0 ) {
	    write_feedback_token (texcoord [0]);
	    write_feedback_token (texcoord [1]);
	    write_feedback_token (texcoord [2]);
