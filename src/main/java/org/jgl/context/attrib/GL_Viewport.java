...
public class GL_Viewport {
	...
	public void set_viewport (int x, int y, int width, int height) {
		...
		Sx = (float)(width-1)/2;	Sy = (float)(height-1)/2;
		Tx = Sx + x;			Ty = Sy + y;
		Size = width * height;
	}
	...
	public float[] Transformation (float NDC []) {
		...
		if (Sx == 0 || NDC [0] == 0) { WinCoord [0] = Tx;
		} else { WinCoord [0] =  Sx * NDC [0] + Tx; }
		if (Sy == 0 || NDC [1] == 0) { WinCoord [1] = Ty;
		} else { WinCoord [1] = -Sy * NDC [1] + Ty; }
		if (Sz == 0 || NDC [2] == 0) { WinCoord [2] = Tz;
		} else { WinCoord [2] =  Sz * NDC [2] + Tz; }
		WinCoord [3] = NDC [3];
	...
	AttribItem.IntPtr   [0] = X;
	AttribItem.IntPtr   [1] = Y;
	AttribItem.IntPtr   [2] = Width;
	AttribItem.IntPtr   [3] = Height;
	AttribItem.FloatPtr [0] = Near;
	AttribItem.FloatPtr [1] = Far;

	AttribItem.FloatPtr [2] = Sx;
	AttribItem.FloatPtr [3] = Sy;
	AttribItem.FloatPtr [4] = Sz;
	AttribItem.FloatPtr [5] = Tx;
	AttribItem.FloatPtr [6] = Ty;
	AttribItem.FloatPtr [7] = Tz;
    }

    public void pop_attrib (gl_list_item AttribItem) {
	X      = AttribItem.IntPtr   [0];
	Y      = AttribItem.IntPtr   [1];
	Width  = AttribItem.IntPtr   [2];
	Height = AttribItem.IntPtr   [3];
	Near   = AttribItem.FloatPtr [0];
	Far    = AttribItem.FloatPtr [1];

	Sx     = AttribItem.FloatPtr [2];
	Sy     = AttribItem.FloatPtr [3];
	Sz     = AttribItem.FloatPtr [4];
	Tx     = AttribItem.FloatPtr [5];
	Ty     = AttribItem.FloatPtr [6];
	Tz     = AttribItem.FloatPtr [7];
    }
	 */
