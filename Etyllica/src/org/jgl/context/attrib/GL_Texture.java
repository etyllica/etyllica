...
import org.jgl.context.attrib.texture.GL_Texture_Gen;
...
public class GL_Texture {
    ...
    public static final int TEXTURE_3D = 4;

    public static final int Q_BIT = 1;
    public static final int R_BIT = 2;
    public static final int S_BIT = 4;
	...
	public boolean check_tex () {
		if (Enable3D) { CurrentObj = Current3D; return true; }
		if (Enable2D) { CurrentObj = Current2D; return true; }
		if (Enable1D) { CurrentObj = Current1D; return true; }
		return false;
	}

	public int is_tex_enabled () {
		int TexEnabled = 0;
		if (Enable1D) TexEnabled = TexEnabled | 1;
		if (Enable2D) TexEnabled = TexEnabled | 2;
		if (Enable3D) TexEnabled = TexEnabled | 4;
		return TexEnabled;
	}

	public boolean is_tex_enabled (int cap) {
		switch (cap) {
		case GL.GL_TEXTURE_1D: return (Enable1D);
		case GL.GL_TEXTURE_2D: return (Enable2D);
	...
	public boolean tex_enable (int state) {
		...
		if ((state & 4)!=0) Enable3D = true; else Enable3D = false;
		return check_tex ();
	}

	public boolean tex_enable (int cap, boolean state) {
		switch (cap) {
		case GL.GL_TEXTURE_1D: Enable1D = state; break;
		case GL.GL_TEXTURE_2D: Enable2D = state; break;
	...
	public int is_tex_gen_enabled () {
		...
		if (CurrentR.Enable) GenEnabled = GenEnabled | 2;
		if (CurrentS.Enable) GenEnabled = GenEnabled | 4;
	...
	public boolean is_tex_gen_enabled (int cap) {
		...
		case GL.GL_TEXTURE_GEN_R: return (CurrentR.Enable);
		case GL.GL_TEXTURE_GEN_S: return (CurrentS.Enable);
	...
	public void tex_gen_enable (int cap, boolean state) {
		...
		case GL.GL_TEXTURE_GEN_R: CurrentR.Enable = state; break;
		case GL.GL_TEXTURE_GEN_S: CurrentS.Enable = state; break;
		case GL.GL_TEXTURE_GEN_T: CurrentT.Enable = state; break;
		}
	}

	public void set_tex (int target, int pname, float params []) {
		switch (target) {
		case GL.GL_TEXTURE_1D: Current1D.set_tex (pname, params); break;
		case GL.GL_TEXTURE_2D: Current2D.set_tex (pname, params); break;
		case GL.GL_TEXTURE_3D: Current3D.set_tex (pname, params); break;
		}
	}

	public void set_tex_image (int target, int level, int border, GL_Image img) {
		switch (target) {
		case GL.GL_TEXTURE_1D: Current1D.set_tex_image(level,border,img); break;
		case GL.GL_TEXTURE_2D: Current2D.set_tex_image(level,border,img); break;
		case GL.GL_TEXTURE_3D: Current3D.set_tex_image(level,border,img); break;
		}
	}

	public GL_Image get_tex_image (int target, int level) {
		switch (target) {
		case GL.GL_TEXTURE_1D: return Current1D.get_tex_image(level);
		case GL.GL_TEXTURE_2D: return Current2D.get_tex_image(level);
		case GL.GL_TEXTURE_3D: return Current3D.get_tex_image(level);
		default: return null;
		}
	}
	...
	public void set_tex_gen_i (int coord, int param) {
		switch (coord) {
		case GL.GL_Q: CurrentQ.Mode = param; break;
		case GL.GL_R: CurrentR.Mode = param; break;
		case GL.GL_S: CurrentS.Mode = param; break;
		case GL.GL_T: CurrentT.Mode = param; break;
		}
	}

	public void set_tex_gen_f (int coord, int pname, float params []) {
		switch (coord) {
		case GL.GL_Q: CurrentQ.set_tex_gen (pname, params); break;
		case GL.GL_R: CurrentR.set_tex_gen (pname, params); break;
		case GL.GL_S: CurrentS.set_tex_gen (pname, params); break;
		case GL.GL_T: CurrentT.set_tex_gen (pname, params); break;
		}
	}

	public void bind_texture (int target, GL_Texture_Obj obj) {
		switch (target) {
		case GL.GL_TEXTURE_1D: Current1D = obj; break;
		case GL.GL_TEXTURE_2D: Current2D = obj; break;
		case GL.GL_TEXTURE_3D: Current3D = obj; break;
		}
		check_tex();
	}
	...
	private int tex_map_nearest_1d (float s, int l) {
		...
		return (0xff000000 | (img.ImageData [i][0][0][0] & 0x000000ff) << 16 |
				(img.ImageData [i][0][0][1] & 0x000000ff) <<  8 |
	...
	private int tex_map_nearest_2d (float s, float t, int l) {
		...
		return (0xff000000 | (img.ImageData [i][j][0][0] & 0x000000ff) << 16 |
				(img.ImageData [i][j][0][1] & 0x000000ff) <<  8 |
	...
	private int tex_map_nearest_3d (float s, float t, float r, int l) {
		...
		return (0xff000000 | (img.ImageData [i][j][k][0] & 0x000000ff) << 16 |
				(img.ImageData [i][j][k][1] & 0x000000ff) <<  8 |
	...
	private int tex_map (float s, float t, float r) {
		if (CurrentObj.MagFilter == GL.GL_NEAREST) {
			if (Enable3D) return tex_map_nearest_3d (s, t, r, 0);
			if (Enable2D) return tex_map_nearest_2d (s, t,    0);
			if (Enable1D) return tex_map_nearest_1d (s,       0);
		} else { // CurrentObj.MagFilter == GL.GL_LINEAR
				if (Enable3D) return tex_map_linear_3d (s, t, r, 0);
				if (Enable2D) return tex_map_linear_2d (s, t,    0);
				if (Enable1D) return tex_map_linear_1d (s,       0);
		}
		return 0;
	}
	...
	private int tex_mid_nearest_nearest (float s, float t, float r, float l) {
		...
		if (Enable3D) return tex_map_nearest_3d (s, t, r, level);
		if (Enable2D) return tex_map_nearest_2d (s, t,    level);
		if (Enable1D) return tex_map_nearest_1d (s,       level);
		return 0;
	}
	...
	private int tex_mid_linear_nearest (float s, float t, float r, float l) {
		...
		if (Enable3D) return tex_map_linear_3d (s, t, r, level);
		if (Enable2D) return tex_map_linear_2d (s, t,    level);
		if (Enable1D) return tex_map_linear_1d (s,       level);
		return 0;
	}
	...
	private int tex_mid_nearest_linear_1d (float s, float l) {
		...
		if (l >= max) {
			...
			return get_mid_linear_color (tex_map_nearest_1d (s, level - 1),
					tex_map_nearest_1d (s, level), l);
		}
	}
	...
	private int tex_mid_linear_linear_1d (float s, float l) {
		...
		if (l >= max) {
			...
			return get_mid_linear_color (tex_map_linear_1d (s, level - 1),
					tex_map_linear_1d (s, level), l);
		}
	}
	...
	private int tex_mid (float s, float t, float r, float l) {
		...
		if (CurrentObj.MagFilter == GL.GL_LINEAR &&
				...
				CurrentObj.MinFilter == GL.GL_LINEAR_MIPMAP_NEAREST)){ c=(float)0.5;
		} else { c = 0; }
	...
	//    private float lambda (float w,float dsdx,float dsdy,float dtdx,float dtdy) {
		private float lambda (float w,float dsdx,float dsdy,float dtdx,float dtdy, float drdx, float drdy) {
			...
			if (dsdx < 0) { dudx = -dsdx; } else { dudx = dsdx; }
			if (dsdy < 0) { dudy = -dsdy; } else { dudy = dsdy; }
			if (dtdx < 0) { dvdx = -dtdx; } else { dvdx = dtdx; }
			if (dtdy < 0) { dvdy = -dtdy; } else { dvdy = dtdy; }
			if (drdx < 0) { dwdx = -drdx; } else { dwdx = drdx; }
			if (drdy < 0) { dwdy = -drdy; } else { dwdy = drdy; }
		...
		public int tex_vertex (float s, float t, float r, float w, float dsdx, float dsdy,
				float dtdx, float dtdy,
				float drdx, float drdy) {
			if (CurrentObj.MinFilter == GL.GL_NEAREST ||
					CurrentObj.MinFilter == GL.GL_LINEAR) {
				return tex_map (s, t, r);
			} else {
		...
		public float [] get_tex_gen_coord (float obj[], float eye[], float nor[]) {
			...
			if (CurrentS.Enable) TexCoord[0] = CurrentS.get_tex_gen_coord(obj, eye, nor);
			else TexCoord[0] = 0;
			if (CurrentT.Enable) TexCoord[1] = CurrentT.get_tex_gen_coord(obj, eye, nor);
			else TexCoord[1] = 0;
			if (CurrentR.Enable) TexCoord[2] = CurrentR.get_tex_gen_coord(obj, eye);
			else TexCoord[2] = 0;
			if (CurrentQ.Enable) TexCoord[3] = CurrentQ.get_tex_gen_coord(obj, eye);
			else TexCoord[3] = 1;
			return TexCoord;
		}
	...
	AttribItem.IntPtr  [0] = ClearIndex;
	System.arraycopy(ClearColor, 0, AttribItem.FloatPtr, 0, 4);
	AttribItem.IntPtr  [1] = IntClearColor;
	AttribItem.IntPtr  [2] = IndexMask;
	AttribItem.IntPtr  [3] = ColorMask;
	AttribItem.BoolPtr [0] = AlphaEnable;
	AttribItem.IntPtr  [4] = AlphaFunc;
	AttribItem.IntPtr  [5] = AlphaRef;
	AttribItem.BoolPtr [1] = BlendEnable;
	AttribItem.IntPtr  [6] = BlendSrc;
	AttribItem.IntPtr  [7] = BlendDst;
	AttribItem.BoolPtr [2] = LogicOPEnable;
	AttribItem.IntPtr  [8] = LogicOPMode;
	AttribItem.BoolPtr [3] = DitherEnable;
    }

    public void pop_attrib (gl_list_item AttribItem) {
	ClearIndex    = AttribItem.IntPtr  [0];
	System.arraycopy(AttribItem.FloatPtr, 0, ClearColor, 0, 4);
	IntClearColor = AttribItem.IntPtr  [1];
	IndexMask     = AttribItem.IntPtr  [2];
	ColorMask     = AttribItem.IntPtr  [3];
	AlphaEnable   = AttribItem.BoolPtr [0];
	AlphaFunc     = AttribItem.IntPtr  [4];
	AlphaRef      = AttribItem.IntPtr  [5];
	BlendEnable   = AttribItem.BoolPtr [1];
	BlendSrc      = AttribItem.IntPtr  [6];
	BlendDst      = AttribItem.IntPtr  [7];
	LogicOPEnable = AttribItem.BoolPtr [2];
	LogicOPMode   = AttribItem.IntPtr  [8];
