...
public class GL_Stipple_Poly_Pixel extends GL_Stipple_Pixel {
	...
	private boolean test (int x, int y) {
		...
		if (glContext.Raster.PolygonStipple) {
			if (((1 << shift) &
					glContext.Raster.PolygonStipplePattern[yindex][xindex]) == 0) {
				return false;
			}
		}
	...
	public void put_pixel (int x, int y, int color) {
		if (test (x, y)) { super.put_pixel (x, y, color); }
	}

	public void put_pixel (int x, int y, float z, int color) {
		if (test (x, y)) { super.put_pixel (x, y, z, color); }
	}
