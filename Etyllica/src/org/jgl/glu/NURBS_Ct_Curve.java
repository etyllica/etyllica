...
public class NURBS_Ct_Curve extends HasGL{

	public NURBS_Ct_Curve(GL gl) {
		super(gl);
	}
	...
	public void draw (NURBS_Curve curv, int i) {
		gl.glMap1f (curv.type, 0, 1, stride, curv.c.order, offsets[i]);
	} 
	...
	public void augment (int s, NURBS_Curve curv) {
		...
		for (i = 0; i < s; i++) {
			for (j = 0; j < curv.c.order; j++) {
				System.arraycopy (ctrl [i*curv.c.order+j], 0,
						offsets [i][j], 0, curv.dim);
			}
		}
	}
