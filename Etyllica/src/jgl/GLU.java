 ...
 *
 * Modified 11/15/2002 by Jon McCall.  Fixed the gluUnProject code.  To fix the
 * gluUnProject method I replaced the inverseMatrix44 with with a different algorithum,
 * it was returning incorrect results.  Also added a method to swap rows and columns
 * (swapRowsCols44) because the projection and model matrixes are expected to be
 * in this format for the gluUnproject algorithum to work correctly.
 */
...
public abstract class GLU extends GL{
	...
	private double det33 (double a1, double a2, double a3,
	 ...
	 * This method replaced by Jon McCall, 11/15/2002 since it was returning
	 * incorrect results.
	 */
	...
	public void gluPerspective (double fovy, double aspect, double zNear, double zFar) {
		...
		glFrustum ((float)xmin, (float)xmax, 
	...
	public boolean gluProject (double objx, double objy, double objz,
	...
	public boolean gluUnProject (double winx, double winy, double winz,
			double model [], double proj [], int viewport [],
	...
	public String gluErrorString (int errorCode) {
		...
		default:
			...
			//		} else {
			if (errorCode >= GLU_NURBS_ERROR1 &&
			errorCode <= GLU_NURBS_ERROR37) {
	...
	private void quadric_error (GLUquadricObj qobj, int error, String msg) {
		...
		if (qobj.ErrorFunc != null) {
			...
			System.err.println("Error with GLUquadricObj");
			/*
	    Object[] arguments = new Object[] { new Integer(error) };
	    try {
		qobj.ErrorFunc.invoke(glJGetComponent(), arguments);
	...
	public void gluQuadricCallback (GLUquadricObj qobj, int which, String fn) {
		if (qobj != null && which == GLU_ERROR) {
			...
			System.err.println("Error with "+fn+".");
			/*
			 try {
				 qobj.ErrorFunc = glJGetComponent().getClass().getMethod(fn, parameterTypes);
	...
	public void gluCylinder (GLUquadricObj qobj, double baseRadius, double topRadius, double height, int slices, int stacks) {
		...
		normal_state = glIsEnabled (GL.GL_NORMALIZE);
		...
		if (qobj.DrawStyle == GLU_POINT) {
			...
			for (i = 0; i < slices; i++) {
				...
				r = baseRadius;
				for (j = 0; j <= stacks; j++) {
					glVertex3f (x * (float)r, y * (float)r, z);
					z += (float)dz;
					r += dr;
				}
			}
			...
			if (qobj.DrawStyle == GLU_LINE) {
				...
				if (baseRadius != 0.0) {
					...
					for (i = 0;i < slices; i++) {
						...
						y = (float)(Math.sin ((double)i * da));
						glNormal3f (x * nsign, y * nsign, nz * nsign);
						glVertex3f (x * (float)baseRadius, y * (float)baseRadius, (float)0.0);
					}
					glEnd ();
					glBegin (GL.GL_LINE_LOOP);
					for (i = 0; i < slices; i++) {
						...
						y = (float)(Math.sin ((double)i * da));
						glNormal3f (x * nsign, y * nsign, nz * nsign);
						glVertex3f (x * (float)topRadius, y * (float)topRadius, (float)height);
					}
					glEnd ();
				}
			}
			...
			for (i = 0; i < slices; i++) {
				...
				for (j = 0; j <= stacks; j++) {
					if (nsign == 1.0) {
						...
						//			if (qobj.TextureFlag) glTexCoord2f(s, t);
						glVertex3f (x1 * (float)r, y1 * (float)r, z);
						glNormal3f (x2 * nsign, y2 * nsign, nz * nsign);
						//			if (qobj.TextureFlag) glTexCoord2f(s, t);
						glVertex3f (x2 * (float)r, y2 * (float)r, z);
					} else {
						glNormal3f (x2 * nsign, y2 * nsign, nz * nsign);
						//			if (qobj.TextureFlag) glTexCoord2f(s, t);
						glVertex3f (x2 * (float)r, y2 * (float)r, z);
		...
		if (normal_state == false) { glDisable (GL.GL_NORMALIZE); }
	}
	...
	public void gluSphere (GLUquadricObj qobj, double radius, int slices, int stacks) {
		...
		if (qobj.DrawStyle == GLU_FILL) {
			...
			glVertex3f ((float)0.0, (float)0.0, nsign * (float)radius);
			for (j = 0; j <= slices; j++) {
				...
				if (normals) { 
					glNormal3f (x * (float)nsign, y * (float)nsign, z * (float)nsign); }
				glVertex3f (x * (float)radius, y * (float)radius, z * (float)radius);
			}
			glEnd();
			...
			glBegin (GL.GL_TRIANGLE_FAN);
			glNormal3f ((float)0.0, (float)0.0, (float)-1.0);
			glVertex3f ((float)0.0, (float)0.0, (float)-radius * nsign);
			...
			glBegin (GL.GL_POINTS);
			if (normals) { glNormal3f ((float)0.0, (float)0.0, nsign); }
			glVertex3f ((float)0.0, (float)0.0, (float)radius);
			if (normals) { glNormal3f ((float)0.0, (float)0.0, -nsign); }
			glVertex3f ((float)0.0, (float)0.0, (float)-radius);
	...
	public void gluDisk (GLUquadricObj qobj, double innerRadius, double outerRadius, int slices, int loops) {
		...
		case GLU_LINE:
			...
			for (l = 0; l <= loops; l++) {
				...
				for (s = 0; s < slices; s++) {
					float a = s * da;
					glVertex2f (r * (float)Math.sin(a), r * (float)Math.cos(a));
				}
				glEnd ();
			}
			/* draw spokes */
		...
		if (qobj.DrawStyle == GLU_POINT) {
			...
			for (loop = 0; loop < loops; loop++) {
				...
				for (slice = 0; slice < slices; slice++) {
					glVertex2d (radius * Math.sin(angle), radius * Math.cos(angle));
					angle += delta_angle;
				}
				radius += delta_radius;
			}
			...
			angle = startAngle*Math.PI/180.0;
			for (slice = 0; slice <= slices; slice++) {
				glVertex2d (outerRadius * Math.sin(angle), outerRadius * Math.cos(angle));
				angle += delta_angle;
			}
			glEnd ();
			/* draw inner ring */
			if (innerRadius > 0.0) {
				...
				angle = startAngle*Math.PI/180.0;
				for (slice = 0; slice < slices; slice++) {
					glVertex2d(innerRadius * Math.sin(angle), innerRadius * Math.cos(angle));
					angle += delta_angle;
				}
				glEnd();
			}
			...
			if (sweepAngle < 360.0) {
				...
				glBegin(GL.GL_LINES);
				glVertex2d(innerRadius * Math.sin(startAngle*Math.PI/180.0),
						innerRadius * Math.cos(startAngle*Math.PI/180.0));
				glVertex2d(outerRadius * Math.sin(startAngle*Math.PI/180.0),
						outerRadius * Math.cos(startAngle*Math.PI/180.0));
				glVertex2d(innerRadius * Math.sin(stopAngle*Math.PI/180.0),
						innerRadius * Math.cos(stopAngle*Math.PI/180.0));
				glVertex2d(outerRadius * Math.sin(stopAngle*Math.PI/180.0),
			...
			for (loop = 0; loop < loops; loop++) {
				...
				for (slice = 0; slice <= slices; slice++) {
					if (qobj.Orientation == GLU_OUTSIDE) {
						glVertex2d((radius + delta_radius) * Math.sin(angle),
	...
	public GLUnurbsObj gluNewNurbsRenderer () {
		return new GLUnurbsObj (this);
	}

	/** GLvoid gluDeleteNurbsRenderer (GLUnurbsObj *nobj) */
	public void gluDeleteNurbsRenderer (GLUnurbsObj nobj) {}

	/** GLvoid gluLoadSamplingMatrices (GLUnurbsObj *nobj,
					const GLfloat modelMatrix [16],
					const GLfloat projMatrix  [16],
					const GLint viewport      [4]) */
	public void gluLoadSamplingMatrices (GLUnurbsObj nobj,
			float modelMatrix [],
			float projMatrix  [],
			int   viewport    []) {
	...
	/** GLvoid gluNurbsProperty (GLUnurbsObj *nobj, GLenum property,
				 GLfloat value) */
	...
	/** GLvoid gluGetNurbsProperty (GLUnurbsObj *nobj, GLenum property,
				    		       GLfloat *value) */
	public void gluGetNurbsProperty (GLUnurbsObj nobj, int property,
			float value []) {
		switch (property) {
		case GLU_SAMPLING_TOLERANCE:
		...
		case GLU_DISPLAY_MODE:
			...
			break;
		case GLU_CULLING:
			if (nobj.culling) { value [0] = (float)1.0; }
			else { value [0] = (float)0.0; }
			break;
		case GLU_AUTO_LOAD_MATRIX:
			if (nobj.auto_load_matrix) { value [0] = (float)1.0; }
			else { value [0] = (float)0.0; }
			break;
		default:
			nobj.glu_error (GLU_INVALID_ENUM);
	...
	/** GLvoid gluNurbsCurve (GLUnurbsObj *nobj,
			      GLint knot_count, GLfloat *knot,
			      GLint stride, GLfloat *ctlarray,
			      GLint order, GLenum type) */
	public void gluNurbsCurve (GLUnurbsObj nobj,
			int knot_count, float knot [],
	...
	/** GLvoid gluNurbsSurface (GLUnurbsObj *nobj,
				GLint sknot_count, GLfloat *sknot,
				GLint tknot_count, GLfloat *tknot,
				GLint s_stride, GLint t_stride,
				GLfloat *ctlarray,
				GLint sorder, GLint torder, GLenum type) */
	public void gluNurbsSurface (GLUnurbsObj nobj,
			int sknot_count, float sknot [],
