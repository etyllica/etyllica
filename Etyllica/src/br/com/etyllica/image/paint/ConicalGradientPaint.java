package br.com.etyllica.image.paint;
...
//            else
      ...
      final java.util.Map<Float, Color> FRACTION_COLORS = recalculate(FRACTION_LIST, colorList, OFFSET);
      ...
      while (ITERATOR.hasNext()) {
        ...
        colorList.add(FRACTION_COLORS.get(CURRENT_FRACTION));
      }
     ...
     * a given rotation
     */
    private java.util.HashMap<Float, Color> recalculate(final List<Float> FRACTION_LIST, final List<Color> COLOR_LIST, final float OFFSET) {
      // Recalculate the fractions and colors with the given offset
      final int MAX_FRACTIONS = FRACTION_LIST.size();
      final java.util.HashMap<Float, Color> FRACTION_COLORS = new java.util.HashMap<Float, Color>(MAX_FRACTIONS);
      for (int i = 0; i < MAX_FRACTIONS; i++) {
